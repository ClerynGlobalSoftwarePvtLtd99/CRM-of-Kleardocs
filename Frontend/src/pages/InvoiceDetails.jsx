  ...
  return (
    <div className="flex-1 p-4 md:p-8 overflow-y-auto w-full max-w-[100vw] text-[var(--color-text-primary)]">
      ...
      {loading ? (
        ...
        <div className="p-8 text-[var(--color-text-secondary)] bg-bg-secondary rounded-2xl border border-bg-tertiary text-center font-bold">
          Invoice not found.
        </div>
      ) : (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1 py-2 border-b border-[var(--color-bg-tertiary)]">
                  <span className="text-[var(--color-text-secondary)] font-medium text-sm text-[11px] uppercase tracking-widest">Invoice No</span>
                  <span className="font-semibold">{inv.invoiceNo}</span>
                </div>
                <div className="flex flex-col gap-1 py-2 border-b border-[var(--color-bg-tertiary)]">
                  <span className="text-[var(--color-text-secondary)] font-medium text-sm text-[11px] uppercase tracking-widest">Invoice Date</span>
                ...
                <div className="flex flex-col gap-1 py-2 border-b border-[var(--color-bg-tertiary)] hover:bg-[var(--color-bg-tertiary)]/50 rounded-md transition-colors">
                  <span className="text-[var(--color-text-secondary)] font-medium text-sm text-[11px] uppercase tracking-widest">Company Name</span>
                  <span className="font-semibold">{inv.customer?.companyName || '-'}</span>
                </div>
                <div className="flex flex-col gap-1 py-2 border-b border-[var(--color-bg-tertiary)]">
                  <span className="text-[var(--color-text-secondary)] font-medium text-sm text-[11px] uppercase tracking-widest">Place of Supply</span>
                  <span className="font-semibold">{inv.placeOfSupply || '-'}</span>
                </div>
              </div>
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1 py-2 border-b border-[var(--color-bg-tertiary)]">
                  <span className="text-[var(--color-text-secondary)] font-medium text-sm text-[11px] uppercase tracking-widest">Customer Name</span>
                  <span className="font-semibold">
                    <Link to={`/customer/${inv.customer?._id}`} className="text-[var(--color-accent)] hover:text-yellow-600 underline underline-offset-4 decoration-[var(--color-bg-tertiary)] hover:decoration-yellow-600 transition-colors">
                      {inv.customer?.name}
                    </Link>
                  </span>
                </div>
                <div className="flex flex-col gap-1 py-2 border-b border-[var(--color-bg-tertiary)]">
                  <span className="text-[var(--color-text-secondary)] font-medium text-sm text-[11px] uppercase tracking-widest">Customer Phone</span>
                  <span className="font-semibold">{inv.customer?.phone}</span>
                </div>
                <div className="flex flex-col gap-1 py-2 border-b border-[var(--color-bg-tertiary)]">
                  <span className="text-[var(--color-text-secondary)] font-medium text-sm text-[11px] uppercase tracking-widest">Emails</span>
                  <span className="font-semibold truncate">{inv.customer?.emails?.join(', ') || '-'}</span>
                </div>
                <div className="flex flex-col gap-1 py-2 border-b border-[var(--color-bg-tertiary)]">
                  <span className="text-[var(--color-text-secondary)] font-medium text-sm text-[11px] uppercase tracking-widest">Customer Address</span>
          ...
          <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-bg-tertiary)] rounded-xl shadow-sm mb-6 overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-[var(--color-bg-tertiary)] text-xs text-[var(--color-text-secondary)] uppercase tracking-widest">
                ...
                <tbody className="divide-y divide-[var(--color-bg-tertiary)]">
                  {inv.items?.map((item, idx) => (
                    <tr key={idx} className="hover:bg-[var(--color-bg-tertiary)]/50 transition-colors">
                      <td className="px-6 py-4 font-medium">{idx + 1}</td>
                      <td className="px-6 py-4 text-sm">{item.hsn}</td>
                      <td className="px-6 py-4 text-[var(--color-accent)] font-bold">{item.service?.name || item.description}</td>
                      <td className="px-6 py-4 text-right whitespace-nowrap font-medium text-sm">{formatCurrency(item.price)}</td>
                      <td className="px-6 py-4 text-right whitespace-nowrap font-bold text-[var(--color-text-primary)]">{formatCurrency(item.amount)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot className="bg-[var(--color-bg-tertiary)]/10">
                  <tr className="font-bold">
                    <td colSpan={4} className="px-6 py-4 text-right text-[var(--color-text-secondary)] text-[11px] uppercase tracking-widest">Total</td>
                    <td className="px-6 py-4 text-right whitespace-nowrap text-lg text-[var(--color-text-primary)]">{formatCurrency(inv.total)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
          ...
          <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-bg-tertiary)] rounded-xl shadow-sm mb-6 w-full">
            <div className="p-4 border-b border-[var(--color-bg-tertiary)] bg-[var(--color-bg-tertiary)]/30 text-[var(--color-accent)] uppercase tracking-widest font-black text-xs">
              Summary
            </div>
            <div className="p-6 flex flex-col gap-3">
              <div className="flex justify-between items-center py-2 border-b border-[var(--color-bg-tertiary)]">
                <span className="font-semibold text-[var(--color-text-secondary)] text-sm">Sub Total</span>
                <span className="font-bold">{formatCurrency(inv.subTotal)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-[var(--color-bg-tertiary)]">
                <span className="font-semibold text-[var(--color-text-secondary)] text-sm">GST Amount</span>
              ...
              <div className="flex justify-between items-center py-2 border-b border-[var(--color-bg-tertiary)] bg-[var(--color-bg-tertiary)]/30 -mx-6 px-6 rounded-md">
                ...
                <span className="font-black text-lg text-[var(--color-accent)]">{formatCurrency(inv.total)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-[var(--color-bg-tertiary)]">
          ...
          {/* Payments History */}
          <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-bg-tertiary)] rounded-xl shadow-sm mb-6 overflow-hidden">
            <div className="p-4 border-b border-[var(--color-bg-tertiary)] bg-[var(--color-bg-tertiary)]/30 uppercase tracking-widest font-black text-xs">
              Payment History
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-[var(--color-bg-tertiary)] text-[10px] text-[var(--color-text-secondary)] uppercase tracking-[0.2em] font-bold">
                ...
                <tbody className="divide-y divide-[var(--color-bg-tertiary)]">
                  {inv.payments?.length > 0 ? inv.payments.map((p, i) => (
                    <tr key={p._id || i} className="hover:bg-[var(--color-bg-tertiary)]/20 transition-colors">
                      <td className="px-6 py-4 font-medium">{formatDate(p.paymentDate)}</td>
                      <td className="px-6 py-4 font-bold text-[var(--color-accent)]">{p.mode}</td>
                      <td className="px-6 py-4 text-xs text-[var(--color-text-secondary)]">{p.note || '-'}</td>
                      <td className="px-6 py-4 text-right font-black text-green-600">{formatCurrency(p.amount)}</td>
                    </tr>
                  )) : (
                    <tr>
                      <td colSpan="5" className="px-6 py-10 text-center text-[var(--color-text-secondary)] italic font-medium opacity-60">
      ...
      {showEmailModal && inv && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-bg-tertiary)] rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[95vh] scale-in-center overflow-hidden">
            {/* Header */}
            <div className="flex justify-between items-center p-6 border-b border-[var(--color-bg-tertiary)] bg-[var(--color-bg-tertiary)]/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[var(--color-text-secondary)] font-bold uppercase tracking-widest">To: {inv.customer?.emails?.[0] || inv.customer?.name}</p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto scrollbar-thin scrollbar-thumb-[var(--color-bg-tertiary)] flex-1 space-y-6">
              ...
              <div className="space-y-2">
                <label className="block text-[11px] font-black uppercase tracking-widest text-[var(--color-text-secondary)] px-1">Subject Line</label>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-3 bg-[var(--color-bg-primary)] border border-[var(--color-bg-tertiary)] rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all text-[var(--color-text-primary)] font-bold text-lg"
              ...
              {/* Variable Chips */}
              <div className="space-y-2">
                <label className="block text-[11px] font-black uppercase tracking-widest text-[var(--color-text-secondary)] px-1 flex items-center gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-[var(--color-bg-primary)] border border-[var(--color-bg-tertiary)] rounded-lg text-xs font-bold text-[var(--color-text-secondary)] hover:border-yellow-500 hover:text-yellow-600 transition-all flex items-center gap-1 group cursor-pointer"
                    >
              ...
              {/* Editor */}
              <div className="space-y-2">
                <label className="block text-[11px] font-black uppercase tracking-widest text-[var(--color-text-secondary)] px-1">Email Content</label>
                <div className="rounded-2xl overflow-hidden border border-[var(--color-bg-tertiary)] bg-[var(--color-bg-primary)] shadow-inner">
                  <RichTextEditor
                    value={emailBody || `
                      <div style="background-color: #f9fafb; padding: 40px 20px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
            ...
            {/* Footer */}
            <div className="p-6 border-t border-[var(--color-bg-tertiary)] bg-[var(--color-bg-tertiary)]/10 flex justify-end gap-3 flex-shrink-0">
              <button
                onClick={() => setShowEmailModal(false)}
      ...
      {showPaymentModal && inv && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-bg-tertiary)] rounded-2xl shadow-2xl p-7 w-full max-w-md flex flex-col scale-in-center">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-xl font-black italic text-[var(--color-accent)] flex items-center gap-2"><CreditCard size={20} /> RECORD PAYMENT</h2>
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-1.5 text-[var(--color-text-secondary)]">Collecting Amount</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-[var(--color-text-secondary)]">₹</span>
                  <input type="number" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} className="w-full pl-8 pr-4 py-3 bg-[var(--color-bg-primary)] border border-[var(--color-bg-tertiary)] rounded-xl focus:outline-none focus:border-[var(--color-accent)] transition-all text-xl font-black text-[var(--color-accent)]" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-1.5 text-[var(--color-text-secondary)]">Internal Note</label>
                <textarea value={paymentNote} placeholder="Reference number, cheque details etc." onChange={(e) => setPaymentNote(e.target.value)} rows={2} className="w-full px-4 py-3 bg-[var(--color-bg-primary)] border border-[var(--color-bg-tertiary)] rounded-xl focus:outline-none focus:border-[var(--color-accent)] transition-all resize-none font-medium"></textarea>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-widest mb-1.5 text-[var(--color-text-secondary)]">Payment Mode</label>
                  <select value={paymentMode} onChange={(e) => setPaymentMode(e.target.value)} className="w-full px-4 py-2 bg-[var(--color-bg-primary)] border border-[var(--color-bg-tertiary)] rounded-xl focus:outline-none focus:border-[var(--color-accent)] transition-all cursor-pointer font-bold">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-widest mb-1.5 text-[var(--color-text-secondary)]">Payment Date</label>
                  <input type="date" value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} className="w-full px-3 py-2 bg-[var(--color-bg-primary)] border border-[var(--color-bg-tertiary)] rounded-xl focus:outline-none focus:border-[var(--color-accent)] transition-all font-bold" />
                </div>
              </div>
