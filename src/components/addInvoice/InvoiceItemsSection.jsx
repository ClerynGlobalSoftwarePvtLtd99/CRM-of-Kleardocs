...
const InvoiceItemsSection = ({ items, setItems }) => {
  ...
  const handleAddProduct = () => {
    ...
    setItems((prev) => [
      ...prev,
      {
        ...
        price: selectedProduct.professional,
        gst: selectedProduct.govt,
        amount: selectedProduct.professional + selectedProduct.govt,
      },
    ])
  ...
  const totalGst = items.reduce((acc, i) => acc + i.gst, 0)
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <tr key={item.id} className="border-b border-[var(--color-bg-tertiary)] hover:bg-[var(--color-bg-tertiary)]/30 transition-colors">
                  <td className="px-5 py-3 text-sm font-medium text-[var(--color-text-secondary)]">{item.no}</td>
                  <td className="px-5 py-3 text-sm font-semibold text-[var(--color-text-primary)]">{item.product.name}</td>
                  <td className="px-5 py-3 text-sm text-right whitespace-nowrap font-medium">₹{item.price.toFixed(2)}</td>
                  <td className="px-5 py-3 text-sm text-right whitespace-nowrap font-medium text-[var(--color-text-secondary)]">₹{item.gst.toFixed(2)}</td>
                  <td className="px-5 py-3 text-sm text-right whitespace-nowrap font-bold text-[var(--color-text-primary)]">₹{item.amount.toFixed(2)}</td>
            ...
            <tr className="bg-[var(--color-bg-tertiary)]/30 border-t border-[var(--color-bg-tertiary)]">
              <td colSpan={2} className="px-5 py-3 text-sm font-bold text-[var(--color-text-secondary)] uppercase tracking-wider">
                Total
              </td>
              <td className="px-5 py-3 text-right whitespace-nowrap font-bold text-[var(--color-text-primary)]">
                ₹{totalPrice.toFixed(2)}
              </td>
              <td className="px-5 py-3 text-right whitespace-nowrap font-bold text-[var(--color-text-secondary)]">
                ₹{totalGst.toFixed(2)}
              </td>
              <td className="px-5 py-3 text-right whitespace-nowrap font-bold text-[var(--color-accent)]">
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.map((p) => (
                    <li
                      ...
                      className={`px-4 py-2.5 text-sm cursor-pointer hover:bg-[var(--color-bg-tertiary)] transition-colors flex justify-between items-center ${
                        selectedProduct?.id === p.id ? 'bg-[var(--color-accent)]/10 text-[var(--color-accent)]' : 'text-[var(--color-text-primary)]'
                      }`}
                    >
